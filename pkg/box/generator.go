...
func main() {
	...
	if _, err := os.Stat(embedFolder); os.IsNotExist(err) {
		log.Fatalf("%s directory does not exists!", embedFolder)
	}
	...
	err := filepath.Walk(embedFolder, func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() {
			...
			if _, err := zw.Write(b); err != nil {
				log.Printf("Error gziping content: %s: %s", path, err)
				return err
			}
			zw.Close()

			// Add file name to map
			configs[relativePath] = gbuf.Bytes()
		}
